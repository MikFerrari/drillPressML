...
exclusions_testP = {'27_11_P_5'}; % try with P_1
...
%     
%     % Train
%     features_selected = feat_unfilt_train(feat_unfilt_train.velocity == labelMap.labelValueP(i),:);
%     
%     numN = size(feat_unfilt_train(feat_unfilt_train.velocity == labelMap.labelValueN(i),:),1);
%     numP = size(feat_unfilt_train(feat_unfilt_train.velocity == labelMap.labelValueP(i),:),1);
%     nsims = numN-numP;
%     
%     features_simulated = bootstrap(features_selected,nsims);
%     feat_unfilt_train= [feat_unfilt_train; features_simulated];
%     
%     % Test
%     features_selected = feat_unfilt_test(feat_unfilt_test.velocity == labelMap.labelValueP(i),:);
%     
%     numN = size(feat_unfilt_test(feat_unfilt_test.velocity == labelMap.labelValueN(i),:),1);
%     numP = size(feat_unfilt_test(feat_unfilt_test.velocity == labelMap.labelValueP(i),:),1);
%     nsims = numN-numP;
%     
%     features_simulated = bootstrap(features_selected,nsims);
%     feat_unfilt_test = [feat_unfilt_test; features_simulated];
%     
% end
