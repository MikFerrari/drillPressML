...
load feat_unfilt_train.mat
load feat_unfilt_test.mat
...
% [trainedClassifier, validationAccuracy] = trainClassifier_filt_NP(feat_unfilt_train);
% [trainedClassifier, validationAccuracy] = trainClassifier_filt_NP_manualFeat(feat_unfilt_train);
% [trainedClassifier, validationAccuracy] = trainClassifier_unfilt_NP(feat_unfilt_train);
[trainedClassifier, validationAccuracy] = trainClassifier_unfilt_NP_manualFeat(feat_unfilt_train);
% save(strcat(modelsPath,'\KNNmodel_filt_NP.mat'),'trainedClassifier');
save(strcat(modelsPath,'\KNNmodel_unfilt_NP.mat'),'trainedClassifier');

% Training Data
pred_vel = trainedClassifier.predictFcn(feat_unfilt_train);
comp = (pred_vel == table2array(feat_unfilt_train(:,end)));
KNN_TrainAccuracy = (length(find(comp)))/length(comp)*100;


train_results = addvars(feat_unfilt_train,pred_vel);
...
% Testing Data
pred_vel = trainedClassifier.predictFcn(feat_unfilt_test);
comp = (pred_vel == table2array(feat_unfilt_test(:,end)));
KNN_TestAccuracy = (length(find(comp)))/length(comp)*100;

test_results = addvars(feat_unfilt_test,pred_vel);
...
savefig(strcat(resultsPath,'\KNN_accuracy_unfilt_',loadFlag,'.fig'))
...
set(gcf,'Visible','off','CreateFcn','set(gcf,''Visible'',''on'')')
savefig(strcat(resultsPath,'\KNN_confusion_unfilt_',loadFlag,'.fig'))
