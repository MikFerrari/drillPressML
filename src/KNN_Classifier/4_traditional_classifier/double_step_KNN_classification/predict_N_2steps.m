load KNNmodel_filt_N
loadFlag = 'N';

pred_vel = trainedClassifier.predictFcn(feat_test_N);
comp = (pred_vel == table2array(feat_test_N(:,end-1)));
KNN_TestAccuracy = (length(find(comp)))/length(comp)*100;

test_results = addvars(feat_test_N,pred_vel);
...
% confMat_N = confusionmat(test_results.velocity,test_results.pred_vel)';
% Transpose in order to keep consistent with the convention used by plotconfusion

actual_N = test_results.velocity;
pred_N = test_results.pred_vel;