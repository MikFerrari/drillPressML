...
[trainedClassifier, validationAccuracy] = trainClassifier_filt_NorP(feat_filt_train);
% [trainedClassifier, validationAccuracy] = trainClassifier_filt_NorP_manualFeat(feat_filt_train);
save(strcat(modelsPath,'\KNNmodel_filt_NorP.mat'),'trainedClassifier');
...
% figure
% plotconfusion(train_results.loadLabel,train_results.pred_vel);
...
test_results = sortrows(test_results,'loadLabel');
...
hold on
plot(test_results.loadLabel,'LineWidth',2)
...
plotconfusion(test_results.loadLabel,test_results.pred_vel);
...
confMat_NorP = confusionmat(test_results.loadLabel,test_results.pred_vel)';
