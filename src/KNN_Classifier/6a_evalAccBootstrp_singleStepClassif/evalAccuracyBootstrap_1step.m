...
title('KNN - Accuracy Classificazione Single-Step')
xlabel('Accuracy')
...
% Italian Plot
figure('numbertitle','off','Visible','off')
plotconfusionTemp(actual,pred);
set(gcf,'Name','Confusion Matrix - Bootstrapping');
title('KNN - Matrice di Confusione')

set(gcf,'Visible','off','CreateFcn','set(gcf,''Visible'',''on'')')
savefig(strcat(resultsPath,'\accuracy_confusion_bootstrap_',loadFlag,'_ITA.fig'))
% exportgraphics(gcf,strcat(resultsPath,'\accuracy_confusion_bootstrap_',loadFlag,'_ITA.pdf'))
% Bad result with exportgraphics, better to print to pdf and crop later